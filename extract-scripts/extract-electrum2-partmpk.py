    ...
    try:
        ...
        if data[:4] == b"BIE1":  # Electrum 2.8+ magic
            raise NotImplementedError(
                "Electrum 2.8+ fully encrypted wallets are supported by btcrecover, but not via data extracts")
        else:
            raise e

wallet_type = wallet.get("wallet_type")
if not wallet_type:
    raise ValueError("Electrum wallet_type not found")
...
if wallet_type == "old":
    if seed_version != 4:
        raise NotImplementedError("Unsupported Electrum1 seed version " + str(seed_version))
    data = base64.b64decode(wallet["seed"])
    if len(data) != 64:
        raise ValueError("Electrum1 encrypted seed plus iv is not 64 bytes long")
    wallet_id = "el"
    ...
    class FoundEncryptedData(BaseException):
        pass
            ...
            # Imported loose private keys
                ...
                for privkey in list(keystore["keypairs"].values()):
                    if privkey:
                        ...
                        wallet_id = "ek"
                        data = privkey[-32:]
                        # only need the 16-byte IV plus the last 16-byte encrypted block of the key
    ...
    except FoundEncryptedData:
        pass

    if xprv:
        data = base64.b64decode(xprv)
        if len(data) != 128:
            raise ValueError("Unexpected Electrum2 encrypted master private key length")
        wallet_id = "e2"
...
l32bytes = wallet_id.encode() + b":" + data
