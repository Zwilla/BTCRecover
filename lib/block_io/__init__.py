...
class BlockIo(object):
    class Key:
        ...
        def from_wif(privkey):
            ...
            is_compressed = len(hexlify(extended_key_bytes)) == 68 and hexlify(extended_key_bytes)[-2:].decode(
                "utf-8") == "01"
    ...
    class Helper:
        ...
        def dynamicExtractKey(user_key, pin):
            ...
            aes_key = BlockIo.Helper.pinToAesKey(pin, algorithm['pbkdf2_salt'], algorithm['pbkdf2_iterations'],
                                                 sha256, algorithm['pbkdf2_phase1_key_length'],
        ...
        def encrypt(data, key, iv=None, cipher_type="AES-256-ECB", auth_data=None):
            ...
            response = {"aes_iv": iv, "aes_cipher_text": None, "aes_auth_tag": None, "aes_auth_data": None,
    ...
    def internal_prepare_sweep_transaction(self, method, **kwargs):
        ...
        self.private_keys[key.pubkey_hex().decode('utf-8')] = PrivateKey(
            secret_exponent=int(key.privkey_hex().decode('utf-8'), 16))
    ...
    def create_and_sign_transaction(self, prepare_data, keys=[]):
        ...
        if self.pin is None and 'user_key' in prepare_data['data'] and prepare_data['data']['user_key'][
            'public_key'] not in self.private_keys:
            raise BlockIoUnknownError("No PIN provided to decrypt signer private key.")

        # decrypt the signer private key if we can
        if self.pin is not None and 'user_key' in prepare_data['data'] and prepare_data['data']['user_key'][
            'public_key'] not in self.private_keys:
            ...
            if (key.pubkey_hex().decode('utf-8') != prepare_data['data']['user_key']['public_key']):
                raise Exception("Expected pubkey=", prepare_data['data']['user_key']['public_key'], "but got pubkey=",
                                key.pubkey_hex(), ". Invalid PIN provided.")
        ...
        for cur_output in outputs:
            tx_output = TxOutput(bitcoinutils.utils.to_satoshis(cur_output['output_value']),
                                 get_output_script(cur_output['receiving_address']))
        ...
        if 'expected_unsigned_txid' in prepare_data['data'] and prepare_data['data'][
            'expected_unsigned_txid'] is not None:
            if prepare_data['data']['expected_unsigned_txid'] != tx.get_txid():
                raise Exception(
                    "Expected unsigned transaction ID mismatch. Please report this error to support@block.io.")
        ...
        for cur_input in inputs:
            ...
            if cur_address_type == 'P2SH' or cur_address_type == 'P2WSH-over-P2SH' or cur_address_type == 'WITNESS_V0':
                ...
                for public_key in cur_public_keys:
                    if (public_key in self.private_keys):
                        if (cur_address_type == 'P2SH'):
                            # P2SH address
                            cur_signatures[public_key] = self.private_keys[public_key].sign_input(tx, cur_input[
                                'input_index'], redeem_script)
                        else:
                            # witness input
                            cur_signatures[public_key] = self.private_keys[public_key].sign_segwit_input(tx,
                                                                                                         cur_input[
                                                                                                             'input_index'],
                                                                                                         redeem_script,
                                                                                                         bitcoinutils.utils.to_satoshis(
                                                                                                             cur_input[
                                                                                                                 'input_value']))
                ...
                if (cur_public_keys[0] in self.private_keys):
                    if cur_address_type == 'P2PKH':
                        # P2PKH address
                        cur_signatures[cur_public_keys[0]] = self.private_keys[cur_public_keys[0]].sign_input(tx,
                                                                                                              cur_input[
                                                                                                                  'input_index'],
                                                                                                              pkh_script)
                    else:
            ...
            for public_key in cur_signatures:
                signatures.append({'public_key': public_key, 'signature': cur_signatures[public_key].decode('utf-8'),
                                   'input_index': cur_input['input_index']})
            ...
            for cur_input in inputs:
                ...
                if cur_address_data['required_signatures'] > 1:
                        ...
                        if (signatures_left > 0):
                            ...
                            if public_key in signatures_dict[str(cur_input_index)]:
                                script_elements.append(
                                    signature_with_sighash(signatures_dict[str(cur_input_index)][public_key]))
                                signatures_left = signatures_left - 1

                    if (signatures_left > 0):
                        raise BlockIoUnknownError("Signatures left should be zero, but signatures_left=",
                                                  signatures_left)
                    ...
                    if cur_address_type == "P2WSH-over-P2SH":
                        ...
                        tx_inputs[cur_input_index].script_sig = Script(
                            [get_output_script(P2wshAddress.from_script(redeem_script).to_string()).to_hex()])
                    ...
                    if cur_address_type == "P2WPKH-over-P2SH":
                        # needs script_sig set as well
                        tx_inputs[cur_input_index].script_sig = Script([get_output_script(
                            PublicKey(cur_public_keys[0]).get_segwit_address().to_string()).to_hex()])
