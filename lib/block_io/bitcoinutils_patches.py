...
bitcoinutils.constants.DEFAULT_TX_VERSION = (1).to_bytes(4, byteorder="little")


# b'\x01\x00\x00\x00' # little-ended version 1
...
def low_r_sign_input(self, tx_digest, sighash=bitcoinutils.constants.SIGHASH_ALL):
    ...
    while True:
        ...
        if counter > 0:
            ...
            # bytearray.fromhex(hex(counter).split("x")[1].rjust(64,"0"))[::-1]
        der_sig = hexlify(
            self.key.sign_digest_deterministic(tx_digest, hashlib.sha256, ecdsa.util.sigencode_der_canonize,
                                               extra_entropy))
...
def get_output_script(address):
    ...
    decoded_bech32 = bitcoinutils.bech32.decode(
        bitcoinutils.constants.NETWORK_SEGWIT_PREFIXES[bitcoinutils_get_network()], address)
    output_script = None
    ...
    if decoded_bech32[0] is None or decoded_bech32[1] is None:
        ...
        correct_checksum = hexlify(
            hashlib.sha256(hashlib.sha256(unhexlify(network_prefix_and_hash160)).digest()).digest())
...
def signature_with_sighash(signature, sighash=bitcoinutils.constants.SIGHASH_ALL):
    ...
    # byte order doesn't matter here since it's just one byte struct.pack('B', sighash))
####
