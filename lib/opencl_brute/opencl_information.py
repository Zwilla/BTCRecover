    ...
    def printfullinfo(self):
        ...
        for i, platformNum in enumerate(cl.get_platforms()):
            ...
            for deviceIndex, device in enumerate(platformNum.get_devices()):
                ...
                print(' Device %d - Constant Memory: {0:.0f} KB'.format(
                    device.max_constant_buffer_size / 1024.0) % deviceIndex)
                print(' Device %d - Global Memory: {0:.0f} GB'.format(
                    device.global_mem_size / 1073741824.0) % deviceIndex)
                print(' Device %d - Max Buffer/Image Size: {0:.0f} MB'.format(
                    device.max_mem_alloc_size / 1048576.0) % deviceIndex)
