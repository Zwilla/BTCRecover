...
def run():
    ...
    # For every server we find in the API, we only take the ones from val country,
    # filter for below given server load, and add them to an array.
    ...
    usableServers.clear()
    print("We have a list of " + str(
        len(nearbyServers)) + " US servers! We will take the top 15 closest, and take the one with the highest ping.")
    index = 0
    ...
    for tempEach in nearbyServers:  # Start iterating through the list
        ...
        if (index == 16):
            break
        ip1 = os.popen(
            'ping ' + tempEach['domain'] + ' -n 1 -w ' + str(timeout))  # Ping the server with a timeout of 150ms
        ...
        if (msLine < bestPing):
            ...
            print(
                "Server " + tempEach['domain'] + " has a faster ping of " + str(bestPing) + ". Setting as best server.")
        else:
    ...
    if (bestServer == "null"):
        input(
            "Critical Error! Server ping was never higher than 9999. This shouldn't ever happen! Check your internet.")
        return
    else:
    ...
    if (savePasswords == True):
        ...
        shutil.copyfile('config_files\\' + bestServer + '.udp.ovpn',
                        configDirectory + '\\config\\' + bestServer + '.udp.ovpn')
